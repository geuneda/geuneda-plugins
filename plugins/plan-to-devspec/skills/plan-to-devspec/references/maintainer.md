...
- [ ] 2번(검토 필요)에서 본문에 `## 검토 요청` 섹션이 카테고리 포맷대로 추가되는지
...
**검토 Q&A 흐름** (검토 필요 → 검토 완료 → 기획 완료)
- [ ] 검토 필요 시 `insert_content`로 본문에 `## 검토 요청 ({날짜})` 섹션이 추가되는지
- [ ] 각 질문이 `### Q{n}. [카테고리]` 형식이고 답변칸 인용줄이 비어 있는지
- [ ] 검토 필요 시 비고가 본문 참조 포인터로 갱신되는지
- [ ] `검토 완료` 상태 항목이 검토 대상보다 먼저 처리되는지
- [ ] **답변 유무 검증을 먼저 수행하는지** (Step 2.5-1)
- [ ] 미답변 질문이 있으면 본문 수정 없이 상태를 `검토 필요`로 되돌리는지
- [ ] 미답변 차단 시 비고에 미답변 질문 번호(`Q2, Q5` 등)가 명시되는지
- [ ] 모든 질문이 답변되면 상태가 `기획 완료`로 변경되고 비고가 확인 마커로 갱신되는지
- [ ] 재검토 시 본문 + 검토 요청 섹션 답변을 함께 읽고 통과/재요청 판정하는지
- [ ] 재검토에서 검토 필요로 떨어질 때 새 `검토 요청` 섹션에 새 항목만 넣는지
