...
```
[데이터베이스가 없습니다]
tc-dev는 노션 TC 데이터소스를 사용합니다. 어떻게 진행할까요?
...
3. 복제 결과로 받은 새 데이터베이스 URL을 `notion-fetch`로 조회해 데이터소스 목록을 얻는다. 데이터소스 중 컬럼 `번호/날짜/내용/선택/우선순위/상태/원인/처리 날짜`가 일치하는 것이 **TC 데이터소스**.
4. 사용자에게 새 데이터베이스 URL과 식별된 데이터소스를 보여주고 진행을 알린다.
5. 옵션 2의 3~5 단계로 이동해 스키마 검증과 설정 저장을 수행한다.

### 옵션 2: 기존 URL 입력 (사용자가 생성)

1. 사용자에게 노션 **TC 데이터베이스 또는 데이터소스 URL**을 묻는다.
...
4. 선택 옵션(`버그`, `개선`), 우선순위 옵션(`당장`, `상`, `중`, `하`), 상태 옵션(`진행 전`, `진행 중`, `완료`, `처리 불가`)이 존재하는지 확인한다. 빠진 게 있으면 사용자에게 노션 UI에서 추가하라고 안내한다.
5. 위 내용을 모두 모아 `.tc-dev-writer.json`에 저장한다 (이 파일은 `tc-dev-resolver`와 공유).
